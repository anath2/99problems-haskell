...
-- The squares on the board are numbered from 1 to n on both x and y axis
...
knightsTo n finish = [pos:path | (pos, path) <- tour (n*n)]
  where 
    tour 1 = [(finish, [])]
    tour k = [(pos', pos:path) | 
                (pos, path) <- tour (k-1), pos' <- sortImage (entrances path)
                        (filter (`notElem` path) (knightMoves n pos))]
        entrances path pos = length (filter (`notElem` path) (knightMoves n pos))
